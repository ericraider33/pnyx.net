...
public static class TextUtil
{
    ...
    public static String? removeParentheses(String? input)
    {
        ...
        int aIndex = input.IndexOf("(", StringComparison.Ordinal);
        while (aIndex >= 0)
        {
            int bIndex = input.IndexOf(")", aIndex, StringComparison.Ordinal);
            if (bIndex < 0)
                return input;

            input = removeParts(input, aIndex, bIndex);
            aIndex = input!.IndexOf("(", StringComparison.Ordinal);
        }
